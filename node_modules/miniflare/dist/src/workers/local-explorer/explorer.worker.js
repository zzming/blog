...
async function fetchFromPeer(peerUrl, apiPath, init) {
  try {
    let url = new URL(`${EXPLORER_API_PATH}${apiPath}`, peerUrl), response = await fetch(url.toString(), {
      ...init,
      headers: {
        ...init?.headers,
...
function getEngineNamespace(env, workflowName) {
  let info = env.LOCAL_EXPLORER_BINDING_MAP.workflows[workflowName];
  return info ? env[info.engineBinding] : null;
}
function getWorkflowBinding(env, workflowName) {
  let info = env.LOCAL_EXPLORER_BINDING_MAP.workflows[workflowName];
  return info ? env[info.binding] : null;
}
function getLocalWorkflows(env) {
  let workflowBindingMap = env.LOCAL_EXPLORER_BINDING_MAP.workflows;
